...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="PubmedArticleSet">
    <html>
    	<head>
    		<meta charset="utf-8"/>
    		<title><xsl:value-of select="PubmedArticle/MedlineCitation/Article/ArticleTitle/text()"/></title>
    	...
    	<body>
    		<article itemscope="itemscope" itemtype="http://schema.org/medicalscholarlyarticle">
    			...
    			<footer>
    				<a href="http://pubmed.gov/{PubmedArticle/PubmedData/ArticleIdList/ArticleId[@IdType='pubmed']}">View at PubMed</a>
    			</footer>
    		</article>
			</body>
    </html>
  ...
  <xsl:template match="ForeName">
    <span itemprop="givenName"><xsl:value-of select="."/></span>
  </xsl:template>

  <xsl:template match="LastName">
    <span itemprop="familyName"><xsl:value-of select="."/></span>
  </xsl:template>
